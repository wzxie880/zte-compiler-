...
# under the License.
"""Additional Transformation Passes. for zte"""
...
def FoldUopLoop():
    ...
    zte support uop programming model
    ...
    def _fold_outermost_loop(body):
        ...
        fail = [False]
        builtin_uop_push = tvm.ir.Op.get("tir.zte.uop_push")
        ...
        def _post_order(op):
            ...
            if op.op.name not in ("tir.zte.command_handle", "tir.tvm_thread_context"):
        ...
        if not fail[0] and all(x is not None for x in gemm_offsets):
            ...
            if not fail[0]:
                begin = tvm.tir.call_extern("int32", "zteUopLoopBegin", stmt.extent, *gemm_offsets)
                end = tvm.tir.call_extern("int32", "zteUopLoopEnd")
    ...
    def _do_fold(stmt):
        ...
        if (
            ...
            and stmt.value.value == env.dev.zte_push_uop.value
        ):
            body = stmt.body
            begins = []
            ends = []
    ...
    return tvm.tir.transform.prim_func_pass(_ftransform, opt_level=0, name="tir.zte.FoldUopLoop")


def CPUAccessRewrite():
    """Detect CPU access to zte buffer and get address correctly.

    zte's buffer is an opaque handle that do not
    correspond to address in CPU.
    This pass detect CPU access and rewrite to use pointer
    returned zteBufferCPUPtr for CPU access.
        ...
        def _post_order(op):
            if isinstance(op, tvm.tir.Allocate):
                ...
                let_stmt = tvm.tir.LetStmt(
                    new_var,
                    tvm.tir.call_extern(
                        "handle", "zteBufferCPUPtr", env.dev.command_handle, buffer_var
                    ),
                    op.body,
                )
    ...
    return tvm.tir.transform.prim_func_pass(
        _ftransform, opt_level=0, name="tir.zte.CPUAccessRewrite"
    )
...
def LiftAllocToScopeBegin():
    ...
    return tvm.tir.transform.prim_func_pass(
        _ftransform, opt_level=0, name="tir.zte.LiftAllocToScopeBegin"
    )
...
def InjectSkipCopy():
    ...
    return tvm.tir.transform.prim_func_pass(_ftransform, opt_level=0, name="tir.zte.InjectSkipCopy")
...
def InjectCoProcSync():
    ...
    def _ftransform(f, *_):
        ...
        def _do_fold(stmt):
            if _match_pragma(stmt, "coproc_sync"):
                success[0] = True
                sync = tvm.tir.Call("int32", "zte.coproc_sync", [])
    ...
    return tvm.transform.Sequential(
        [
            tvm.tir.transform.prim_func_pass(_ftransform, 0, "tir.zte.InjectCoProcSync"),
            tvm.tir.transform.CoProcSync(),
        ],
        opt_level=0,
        name="tir.zte.InjectCoProcSync",
    )
...
def InjectDMAIntrin():
    ...
    #     # buf.strides.append(1)
    #     # buf.shape.append(16)
    #     # print("********** start fold ***********")
    #     # print("scope is ", scope)
    #     # print("type_buf.strides", type(buf.strides))
    #     # print("type_buf.strides[0]", type(buf.strides[0]))
    #
    #     ndim = len(buf.shape)
    #     x_size = 1
    #     base = 0
    ...
    #     buf_shape = []
    #
    #     if scope == "local.input_bufferA":
    #         for i in range(0, ndim-1):
    #             buf_strides.append(buf.strides[ndim-1-i])
    #
    #         buf_strides = list(reversed(buf_strides))
    #         # print("buf_strides", buf_strides)
    #
    #         for i in range(0, ndim):
    #             buf_shape.append(buf.shape[ndim-1-i])
    #         # print("buf_shape", buf_shape)
    #     else:
    #         #buf_strides = buf.strides
    #         buf_shape = buf.shape
    #
    #     for i in range(1, ndim + 1):
    #
    #         # print("*********************")
    #         # print(buf_strides[ndim - i] - x_size)
    #         # print("judge", utils.equal_const_int(buf_strides[ndim - i] - x_size, 0))
    #         # print("buf_strides[ndim - i]", buf_strides[ndim - i])
    #         # print("buf_strides",buf_strides)
    #         # print("x_size", x_size)
    #         # print("i", i)
    #         # print("ndim", ndim)
    #         # print("scope", scope)
    #         # print("buf.strides", buf.strides)
    #         # print("buf", buf)
    #
    #         if not utils.equal_const_int(buf_strides[ndim - i] - x_size, 0):
    #             raise RuntimeError("scope %s needs to have stride block=%d" % (scope, elem_block))
    #         x_size = x_size * buf_shape[ndim - i]
    #
    #         # print("buf.shape", buf.shape)
    #         # print("x_size", x_size)
    #         # print("elem_block",elem_block)
    ...
    #     # print("shape", shape, "buf.shape", buf.shape)
    #     return shape, strides

    def _get_2d_pattern(buf, elem_width, elem_bytes, dtype, scope, allow_fold):
        # elem_block = 8
        # elem_block = elem_bytes * 2 // elem_width
        #elem_block=16
        # print("elem_bytes", elem_bytes)
        # print("elem_width", elem_width)
        # print("buf.elem_offset", buf.elem_offset)
        # print("idxm",idxm(buf.elem_offset, elem_block))
        print("dtype",dtype)
        ...
        if flat:
            ...
            if ndim == 4:
                ...
                x_size2 = shape[-4]
                x_size3 = 0
                y_size= x_size0 * x_size1 * x_size2
                return x_size0, x_size1, x_size2, x_size3, y_size,base
            ...
            if ndim == 5:
                ...
                x_size2 = shape[-4]
                x_size3 = shape[-5]
                y_size= x_size0 * x_size1 * x_size2 * x_size3
                return x_size0, x_size1, x_size2, x_size3, y_size,base
    ...
    def _inject_copy(src, dst, pad_before, pad_after, pad_value):
        ...
        mask=0
        # print("src",src)
        # #print("src_addr",src.addr)
        # print("src.data",src.data)
        # print("dst",dst)
        # print("dst.data*********", dir(dst))

        print("src.scope", src.scope)
        print("dst.scope", dst.scope)
        if(src.dtype=="int8"):
            base0 = 8
        else:
            base0 = 2
        ...
        if dst.scope == "global":
            ...
            if src.scope == env.mid_scopeA:
                ...
                mem_type = env.dev.MEM_ID_INPUTA
                data_type = "int%d" % env.INP_WIDTH
                task_qid = env.dev.QID_LOAD_INP
            elif src.scope == env.input_scopeB:
                elem_width = env.INP_WIDTH
                elem_bytes = env.INPUT_ELEM_BYTES*9
                mem_type = env.dev.MEM_ID_INPUTB
                data_type = "int%d" % env.INP_WIDTH
                task_qid = env.dev.QID_LOAD_INP
            elif src.scope == env.output_scopeA:
                elem_width = env.OUT_WIDTH
                elem_bytes = env.OUT_ELEM_BYTES*8
                mem_type = env.dev.MEM_ID_OUT
            ...
            # _check_compact(src)
            # print("inject_copy_elem_bytes", elem_bytes)
            # print("src",src)
            # print(dir(src))
            # print(type(src))
            # print("src.name", src.name)
            # print("src.offset_factor", src.offset_factor)
            # print("src.data_alignment", src.data_alignment)
            # print("sernoti.strides", src.strides)
            # print("src.handle", src.handle)
            # print("src.scope",src.scope)
            # # print(tvm.tir.Var(dst.handle.value, "int64"))
            # pb = tvm.tir.Var(name="pb", dtype="int64")
            # print("*******", dir(pb))
            x_size0, x_size1 , x_size2, x_size3, y_size, base= _get_2d_pattern(
                dst, elem_width, elem_bytes, data_type, src.scope, allow_fold=True
            )
            #x_size=x_size.astype(uint64)
            # print(" src.access_ptr" ,src.access_ptr("r", "int32"))
            ...
            elif src.scope == env.mid_scopeA:
                print("fuck", dst.access_ptr("rw", "int64"))
                fuck = dst.access_ptr("rw", "int64")
                irb.emit(
                    tvm.tir.call_extern(
                        "int64",
                        "store",
            ...
            if dst.scope == env.inp_scope:
                elem_width = env.INP_WIDTH
                elem_bytes = env.INP_ELEM_BYTES * 8
                mem_type = env.dev.MEM_ID_INP
                data_type = "int%d" % env.INP_WIDTH
                task_qid = env.dev.QID_LOAD_INP
        ...
        elif src.scope == env.param_scopeA:
            if dst.scope == env.inp_scope:
                elem_width = env.INP_WIDTH
                elem_bytes = env.INP_ELEM_BYTES * 8
                mem_type = env.dev.MEM_ID_INP
            ...
            # print(type(src.data), "type(src.data)")
            # print(type(src.handle.value), "type(src.handle.value)")
            # print(type(tvm.tir.const(0x10000000, "int64")), "tvm.tir.const")
            # if dst.scope == env.inp_scope:
            #     irb.emit(
            #         tvm.tir.call_extern(
            #             "int64",
            #             "move",
            #             # tvm.tir.const(src.handle.value, "int64"),
            #             tvm.tir.const(0x20000000, "int64") + src.access_ptr("rw", "int64") * 4,
            #             # dst.data,
            #             tvm.tir.const(0x20020000, "int64") + dst.access_ptr("rw", "int64") * 2,
            #             # x_size0,
            #             # x_size1,
            #             # x_size2,
            #             # x_size3,
            ...
            #             "move",
            #             # tvm.tir.const(src.handle.value, "int64"),
            #             tvm.tir.const(0x20000000, "int64"),
            #             # dst.data,
            #             tvm.tir.const(0x20040000, "int64"),
            #             # x_size0,
            #             # x_size1,
            ...
            #         )
            #     )
            if dst.scope == env.mid_scopeA:
                irb.emit(
                    tvm.tir.call_extern(
                        "int64",
                        "move",
                        # tvm.tir.const(src.handle.value, "int64"),
                        tvm.tir.const(0x20005000, "int64"),
                        # dst.data,
                        tvm.tir.const(0x20060000, "int64") ,
                        # x_size0,
                        # x_size1,
        ...
        elif src.scope == env.param_scopeB:
            if dst.scope == env.inp_scope:
                elem_width = env.INP_WIDTH
                elem_bytes = env.INP_ELEM_BYTES * 8
                mem_type = env.dev.MEM_ID_INP
            ...
            #         )
            #     )
            if dst.scope == env.mid_scopeB:
                irb.emit(
                    tvm.tir.call_extern(
                        "int64",
                        "move",
                        # tvm.tir.const(src.handle.value, "int64"),
                        tvm.tir.const(0x20015000, "int64"),
                        # dst.data,
                        tvm.tir.const(0x20070000, "int64") ,
                        # x_size0,
                        # x_size1,
            ...
            if dst.scope == env.inp_scope:
                ...
                irb.emit(
                    tvm.tir.call_extern(
                        ...
                        tvm.tir.const(0x20000000, "int64"),
                        # dst.data,
                        tvm.tir.const(0x20060000, "int64") + dst.access_ptr("rw", "int64")*8,
                        # x_size0,
                        # x_size1,
                        # x_size2,
                        # x_size3,
            ...
            #             "move",
            #             # tvm.tir.const(src.handle.value, "int64"),
            #             tvm.tir.const(0x20010000, "int64"),
            #             # dst.data,
            ...
            if dst.scope == env.wgt_scope:
                irb.emit(
                    tvm.tir.call_extern(
                        ...
                        "move",
                        # tvm.tir.const(src.handle.value, "int64"),
                        tvm.tir.const(0x20010000, "int64")+src.access_ptr("rw", "int64")*4,
                        # dst.data,
                        tvm.tir.const(0x20040000, "int64")+dst.access_ptr("rw", "int64")*16,
                        # x_size0,
                        # x_size1,
                        # x_size2,
                        # x_size3,
                ...
                irb.emit(
                    tvm.tir.call_extern(
                        ...
                        tvm.tir.const(0x20010000, "int64")+src.access_ptr("rw", "int64")*4,
                        # dst.data,
                        tvm.tir.const(0x20070000, "int64")+dst.access_ptr("rw", "int64")*16,
                        # x_size0,
                        # x_size1,
                        # x_size2,
                        # x_size3,
            ...
            if dst.scope == env.mid_scopeA:
                ...
                data_type = "int%d" % env.WGT_WIDTH
                task_qid = env.dev.QID_LOAD_WGT
            elif dst.scope == env.input_scopeA:
                elem_width = env.INP_WIDTH
                elem_bytes = env.INPUT_ELEM_BYTES
                mem_type = env.dev.MEM_ID_INPUTA
                data_type = "int%d" % env.INP_WIDTH
                task_qid = env.dev.QID_LOAD_INP
            elif dst.scope == env.input_scopeB:
                elem_width = env.INP_WIDTH
                elem_bytes = env.INPUT_ELEM_BYTES
                mem_type = env.dev.MEM_ID_INPUTB
                data_type = "int%d" % env.INP_WIDTH
                task_qid = env.dev.QID_LOAD_INP
            elif dst.scope == env.output_scopeA:
                elem_width = env.OUT_WIDTH
                elem_bytes = env.OUT_ELEM_BYTES * 8
                mem_type = env.dev.MEM_ID_OUT
                data_type = "int%d" % env.OUT_WIDTH
                task_qid = env.dev.QID_STORE_OUT
            elif dst.scope == env.output_scopeB:
                elem_width = env.OUT_WIDTH
                elem_bytes = env.OUT_ELEM_BYTES * 8
                mem_type = env.dev.MEM_ID_OUT
                data_type = "int%d" % env.OUT_WIDTH
            ...
            print(type(src.handle.value), "type(src.handle.value)")
            print(type(tvm.tir.const(0x10000000, "int64")), "tvm.tir.const")
            if dst.scope == env.output_scopeA:
                irb.emit(
                    tvm.tir.call_extern(
                        "int64",
                        "move",
                ...
                irb.emit(
                    tvm.tir.call_extern(
                        ...
                        tvm.tir.const(0x20060000, "int64"),
                        # dst.data,
                        tvm.tir.const(0x20070000, "int64")+dst.access_ptr("rw", "int64")*8,
                        # src.offset,
                        # dst.offset,
                        # x_size0,
                        # x_size1,
                ...
                irb.emit(
                    tvm.tir.call_extern(
                        ...
                        "move",
                        # tvm.tir.const(src.handle.value, "int64"),
                        tvm.tir.const(0x20060000, "int64")+src.access_ptr("rw", "int64")*8,
                        # dst.data,
                        tvm.tir.const(0x20000000, "int64")+dst.access_ptr("rw", "int64")*4,
                        # src.offset,
                        # dst.offset,
                        # x_size0,
                        # x_size1,
            ...
            if dst.scope == env.output_scopeA:
                ...
                irb.emit(
                    tvm.tir.call_extern(
                        ...
                        "move",
                        # tvm.tir.const(src.handle.value, "int64"),
                        tvm.tir.const(0x20070000, "int64")+src.access_ptr("rw", "int64")*8,
                        # dst.data,
                        tvm.tir.const(0x20010000, "int64")+dst.access_ptr("rw", "int64")*4,
            ...
            if dst.scope == env.mid_scopeA:
                ...
                data_type = "int%d" % env.INP_WIDTH
                task_qid = env.dev.QID_LOAD_INP
            elif dst.scope == env.param_scopeA:
                elem_width = env.INP_WIDTH
                elem_bytes = env.INPUT_ELEM_BYTES
                mem_type = env.dev.MEM_ID_INPUTA
                data_type = "int%d" % env.INP_WIDTH
                task_qid = env.dev.QID_LOAD_INP
            elif dst.scope == env.param_scopeB:
                elem_width = env.INP_WIDTH
                elem_bytes = env.INPUT_ELEM_BYTES
                mem_type = env.dev.MEM_ID_INPUTB
                data_type = "int%d" % env.INP_WIDTH
...
def InjectConv2DTransposeSkip():
        ...
        def _do_fold(op):
            if _match_pragma(op, "conv2d_transpose_gemm"):
                ...
                if is_init:
                    ...
                    irb = tvm.tir.ir_builder.create()
                    dev = env.dev
                    irb.scope_attr(dev.zte_axis, "coproc_scope", dev.get_task_qid(dev.QID_COMPUTE))
                    irb.scope_attr(dev.zte_axis, "coproc_uop_scope", dev.zte_push_uop)
                    irb.emit(
                        tvm.tir.call_intrin(
                            "int32",
                            "tir.zte.uop_push",
                    ...
                    with irb.if_scope(condition):
                        dev = env.dev
                        irb.scope_attr(
                            dev.zte_axis, "coproc_scope", dev.get_task_qid(dev.QID_COMPUTE)
                        )
                        irb.scope_attr(dev.zte_axis, "coproc_uop_scope", dev.zte_push_uop)
                        irb.emit(
                            tvm.tir.call_intrin(
    ...
    return tvm.tir.transform.prim_func_pass(
        _ftransform, opt_level=0, name="tir.zte.InjectConv2DTrasnposeSkip"
    )
...
def AnnotateALUCoProcScope():
    ...
    def _ftransform(func, mod, ctx):
        ...
        def _do_fold(stmt):
            if _match_pragma(stmt, "alu"):
                irb = tvm.tir.ir_builder.create()
                irb.scope_attr(
                    env.dev.zte_axis, "coproc_scope", env.dev.get_task_qid(env.dev.QID_COMPUTE)
                )
                irb.scope_attr(
                    env.dev.zte_axis, "coproc_uop_scope", tvm.tir.StringImm("ztePushALUOp")
                )
                irb.emit(stmt)
                return irb.get()
    ...
    return tvm.tir.transform.prim_func_pass(
        _ftransform, opt_level=0, name="tir.zte.AnnotateALUCoProcScope"
    )
...
def InjectALUIntrin():
    ...
    def _ftransform(func, mod, ctx):
        ...
        def _do_fold(stmt):
            ...
            print("*******************************************************")
            print("stmt_type",type(stmt))
            def _flatten_loop(src_coeff, dst_coeff, extents):
                print("*******************************************************")
                print("dst_coeff",dst_coeff)
                print("src_coeff",src_coeff)
            ...
            if _match_pragma(stmt, "alu"):
                ...
                print("loop_body",loop_body)
                print(type(loop_body))
                print("loop_body_dir",dir(loop_body))
                # print("loop_body.seq", loop_body.seq[1])
                # print("loopbody_type",type(loop_body))
                # # print("loop_body.index",loop_body.index)
                # # print("loop_body.value",loop_body.value)
                # print("loop_body.span", loop_body.span)
                # print("getattr_loop_body", loop_body.handle)
                # print("stmt_dir",dir(stmt))
                dst_var = loop_body.buffer_var
                #dst_var = placeholder.local.acc_buffer: Pointer(float32)
                ...
                for _ in range(nest_size):
                    print("tmp_body",tmp_body)
                    if isinstance(tmp_body.body, tvm.tir.stmt.SeqStmt):
                        tmp_body.body = tmp_body.body.seq[1]
            ...
            #     # Derive array index coefficients
                ...
                print("dst_coeff0",dst_coeff)
                # print("src_coeff", src_coeff)
                print(type(dst_coeff[0]))
                # Check if lhs/rhs is immediate
                use_imm = False
                imm_val = None
                print("rhs",rhs)
                print("lhs",lhs)
                print("tvm.tir.IntImm",tvm.tir.IntImm)
                print("loop_body.value",loop_body.value)
                print("dst_var",dst_var)
                # print("lhs.buffer_var",lhs.buffer_var)
                # print("rhs.buffer_var",rhs.buffer_var)
                print("tvm.tir.IntImm",tvm.tir.IntImm)
                # print(lhs.buffer_var.same_as(dst_var))
                ...
                #     irb.emit(tvm.tir.call_extern("int32", "zteUopLoopEnd"))
                return irb.get()
            return stmt
    ...
    return tvm.tir.transform.prim_func_pass(
        _ftransform, opt_level=0, name="tir.zte.InjectALUIntrin"
    )
